//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace Esmart.Infrastructure.Data
{
    using System;
    using System.Collections.Generic;
    
    public partial class tbl_PQShippingDetails
    {
        public long PQShip_Id { get; set; }
        public string Recived_Company { get; set; }
        public string Contact_Person { get; set; }
        public string Mobile { get; set; }
        public string Land_Line { get; set; }
        public string Fax { get; set; }
        public string CompanyTax_ID { get; set; }
        public string Mode_Shipment { get; set; }
        public string Port_Origon { get; set; }
        public string Port_Destination { get; set; }
        public string Shipment_Terms { get; set; }
        public Nullable<System.DateTime> Date_Delivery { get; set; }
        public Nullable<long> country_id { get; set; }
        public Nullable<long> State_id { get; set; }
        public string City { get; set; }
        public string Delivery_Address { get; set; }
        public string Delivery_Terms { get; set; }
        public string Remarks { get; set; }
        public string updateuser_name { get; set; }
        public Nullable<System.TimeSpan> sys_time { get; set; }
        public Nullable<System.DateTime> sys_date { get; set; }
        public Nullable<long> PQuotation_Id { get; set; }
        public Nullable<bool> del_status { get; set; }
        public string PS_Type { get; set; }
    }
}
